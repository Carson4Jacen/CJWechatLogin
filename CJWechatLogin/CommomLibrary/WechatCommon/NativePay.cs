...
namespace WxPayAPI
{
    public class NativePay
    {
        ...
        */
        ...
        //模式2封装-Cboyce 2016年8月12日
        public string TrunPayUrl(string productId, string body,string attach,int total_fee,string goods_tag,int expires)
        {
            Log.Info(this.GetType().ToString(), "Native pay mode 2 url is producing...");
            ...
            data.SetValue("time_start", DateTime.Now.ToString("yyyyMMddHHmmss"));//交易起始时间
            data.SetValue("time_expire", DateTime.Now.AddMinutes(expires).ToString("yyyyMMddHHmmss"));//交易结束时间
