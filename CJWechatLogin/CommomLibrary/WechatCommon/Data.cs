        ...
        **/
        public string ToXml()
        {
            ...
            if (0 == m_values.Count)
            {
                ...
                throw new WeChatException("WxPayData数据为空!");
            }
            ...
            foreach (KeyValuePair<string, object> pair in m_values)
            {
                ...
                else//除了string和int类型不能含有其他数据类型
                {
                    ...
                    throw new WeChatException("WxPayData字段数据类型错误!");
                }
            }
            xml += "</xml>";
            return xml;
        ...
        */
        public SortedDictionary<string, object> FromXml(string xml)
        {
            if (string.IsNullOrEmpty(xml))
            {
                ...
                throw new WeChatException("将空的xml串转换为WxPayData不合法!");
            }
            ...
            {
                throw new WeChatException(ex.Message);
            }
        ...
        public bool CheckSign()
        {
            ...
            if (!IsSet("sign"))
            {
               //Log.Error(this.GetType().ToString(), "WxPayData签名存在但不合法!");
                throw new WeChatException("WxPayData签名存在但不合法!");
            }
            //如果设置了签名但是签名为空，则抛异常
           ...
           // Log.Error(this.GetType().ToString(), "WxPayData签名验证错误!");
            throw new WeChatException("WxPayData签名验证错误!");
        }
